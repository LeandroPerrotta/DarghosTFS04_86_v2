...
function pvpBattleground.getInformations()
	local msg = ""
	msg = msg .. "Este � um sistema de PvP do Darghos, e o objetivo � seu time atingir 50 pontos, obtidos ao derrotar um oponente. A partida tem dura��o de at� 15 minuto\n"
	...
	msg = msg .. "Aos participantes do time vencedor � concedido uma quantidade de pontos de experiencia e algum dinheiro!\n"
	...
	msg = msg .. "Use o PvP Channel para se comunicar com seus companheiros, somente eles poder�o ler suas mensagens. Boa sorte!\n"
...
function pvpBattleground.broadcastLeftOnePlayer()
	...
	local messages = {
		...
		"N�o conhe�e o sistema de Battlegrounds? Conhe�a agora! Falta ap�nas voc� para o proxima batalha 6x6! N�o h� perdas nas mortes, ajude o time na vitoria e ganhe recompensas! -> !bg entrar",
	}
...
function pvpBattleground.onEnter(cid)
	...
	if(ret == BG_RET_INFIGHT) then
		doPlayerSendCancel(cid, "Voc� est� em condi��o de batalha, aguarde sair a condi��o e tente novamente.")
		return false
	end
	...
	if(ret == BG_RET_PUT_IN_WAITLIST) then
		...
		if(not closeTeam) then
			...
			pvpBattleground.sendPvpChannelMessage("[Battleground] Os times para a proxima battleground est�o completos! A nova partida come�ar� em instantes, assim que a Battleground estiver vazia...", PVPCHANNEL_MSGMODE_OUTBATTLE)
		end
...
function pvpBattleground.onReportIdle(cid, idle_player)
	...
	if(getBattlegroundStatus() ~= BATTLEGROUND_STATUS_STARTED) then
		pvpBattleground.sendPlayerChannelMessage(cid, "Somente � permitido fazer den�ncias ap�s a Battleground ter iniciado.")
		return
	end	
	...
	if(report_block ~= 0 and os.time() <= report_block) then
		pvpBattleground.sendPlayerChannelMessage(cid, "Voc� est� impossibilitado de efetuar denuncias de jogadores inativos momentaneamente por uma denuncia invalida recente.")
		return
	end
