...
function hasAllPieces(cid, pieces)
	...
	for k,v in pairs(pieces) do
		...
		return (found == #pieces) and true or false
	end
end
    	...
    	if(not alreadyHasAmulet) then
    		if(hasAllPieces) then
    			if(amuletDate == -1) then
    				npcHandler:say("Oooohhh! Quanto tempo eu não via isto? Todas as partes reunidas! Magnifico trabalho! Se você me entregar-las e mais 5000 moedas de ouro para amanha eu lhe entregarei o amuleto do velho Koshei, aceita?")
    			elseif(amuletDate + (60 * 60 * 24) > os.time()) then
    				npcHandler:say("Mas ainda não se passou um dia, o seu amuleto ainda não está pronto! Deixe-me trabalhar ou irá atrazar!")
    				npcHandler:resetNpc()
				else
					...
					npcHandler:say("Que bom lhe ver, por que tanta demora?? Tenho boas noticias! Seu amuleto está pronto! Aqui está! Espero que faça bom aproveito!")
    			end
    		else
    			npcHandler:say("Então quer saber do amuleto? Ohh.. Ok! Estamos falando de um artefato muito poderoso, o amuleto de Koshei, uma criatura que acreditam ser imortal! E eu posso o fazer, você o gostaria?")
    		end		
    	else
    		npcHandler:say("Mas que pena, não conheço outros amuletos especiais além do que já lhe fiz...")
    		npcHandler:resetNpc()
    	end
    elseif(talk_state == 2) then
    	if(hasAllPieces) then
    		if(takeAllPieces(cid, AMULET_PIECES)) then
    			...
    			playerSetStorageValue(cid, sid.KOSHEI_AMULET_DATE, os.time())
    			npcHandler:say("Otimo, agora mê de um dia para trabalhar na restauração, retorne amanha e lhe entregarei o amuleto!")
			else
				npcHandler:resetNpc()
				return false
    		end
    	else
    		npcHandler:say("Bom, para conseguir-lo não será facil, pois o amuleto foi partido em quatro partes que estão espalhadas por todo deserto de Salazart. Mas eu sei o lugar aproximado em que elas estão, você precisará buscar-las, certo?")
    	end 	
    elseif(talk_state == 3) then
    	npcHandler:say("Preste atenção: O paradeiro da primeira parte é uma camara no sub-solo não muito longe daqui ao sudoeste, proximo a entrada para o barco da cidade, procure-o nas raizes do chão. Para a segunda parte siga ao sudeste, procure na proa do barco dos piratas esqueletos.")
    	npcHandler:say("Para a terceira parte siga ao norte, e você encontrará um pequeno oásis, procure nas arvores. E finalmente para a quarta e última parte siga ao noroeste pela margem do oceano, logo quando este der lugar a montanha examine as rochas. Retorne quando estiver com todas as partes.")
    end
